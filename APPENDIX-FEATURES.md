...
| `mise prepare` | Pre-download tools without installing (CI cache warming) | `mise prepare` | [link](https://mise.jdx.dev/cli/prepare.html) |
| Tool-level `postinstall` | Run a script immediately after a specific tool installs | `node = { version = "24", postinstall = "npm i -g pnpm" }` | [link](https://mise.jdx.dev/hooks.html#tool-level-postinstall) |
| `install_before` | Only resolve versions older than N days (supply chain security) | `[settings] install_before = "7d"` | [link](https://mise.jdx.dev/dev-tools/mise-lock.html#minimum-release-age) |
| Tool aliases | Define short aliases for tools globally | `mise tool-alias set jdk java` | [link](https://mise.jdx.dev/cli/tool-alias.html) |
| Tool stubs | Generate `bin/<tool>` wrapper scripts for tools | `mise generate tool-stub ./bin/gh` | [link](https://mise.jdx.dev/cli/generate/tool-stub.html) |
...
| Task templates | Use Tera templating in task scripts | `run = "echo {{env.APP_ENV}}"` | [link](https://mise.jdx.dev/tasks/templates.html) |
...
| Task sandboxing | Run tasks in an isolated environment | `[tasks.safe] sandbox = true` | [link](https://mise.jdx.dev/sandboxing.html) |
...
| GitHub token | Avoid GitHub API rate limits | `mise token github` | [link](https://mise.jdx.dev/dev-tools/github-tokens.html) |
...
The codelab slides are built with [reveal.js 6.0](https://revealjs.com) via [reveal-md 6.1.4](https://github.com/webpro/reveal-md).

- Source: `slides/slides.md` — pure Markdown, version-controlled
- Theme: `slides/theme/brigade.css` — custom dark kitchen theme
- Config: `slides/reveal.json`
...
| `mise run slides:pdf` | Export PDF via Puppeteer |
