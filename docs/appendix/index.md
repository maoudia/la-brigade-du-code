...
The codelab slides are built with [reveal-md 6.1.4](https://github.com/webpro/reveal-md) from a single Markdown source file.

- Source: `slides/slides.md` — single source of truth, version-controlled
- Theme: `slides/theme/brigade.css` — custom dark kitchen theme
- Built output: `slides/_dist/` — generated by `mise run slides:build` (gitignored)
- Live: https://maoudia.github.io/la-brigade-du-code/slides/

| Task | What it does |
|---|---|
| `mise run slides:install` | Install reveal-md dev dependency |
| `mise run slides:dev` | Hot-reload dev server on :1948 |
| `mise run slides:build` | Compile `slides.md` → `slides/_dist/` |
| `mise run slides:pdf` | Export to PDF via reveal-md |
