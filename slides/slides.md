...
🗑️ `sdkman` — Java version manager

🗑️ `direnv` — env var scoping

🗑️ `Makefile` — task runner

🗑️ `npm scripts` — more task runner

<div class="solution-badge">One file. Every OS. Every developer.</div>
...
<!-- .slide: data-background-color="#0F1117" style="font-size: 82%" -->
...
<!-- .slide: data-background-color="#0F1117" data-auto-animate style="font-size: 82%" -->
...
mise lock && cat mise.lock
mise doctor && mise run brigade
```
...
mise env | grep VITE
cd backend
mise env | grep SERVER_PORT
cd ../frontend
mise env | grep VITE_API_URL
```
